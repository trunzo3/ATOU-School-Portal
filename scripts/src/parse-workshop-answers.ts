 ...
 * app's structured teacher rows.
 *
 * Real formats seen in the sheet:
 *   "Beardsley 4th 32 students"        → Beardsley, 32   (grade dropped)
 *   "Randazzo (room 27)- 29"           → Randazzo, 29    (parenthetical dropped)
 *   "Callie Cyr - 27"                  → Callie Cyr, 27
 *   "Shirley Fernandez - (29)"         → Shirley Fernandez, 29
 *   "Katie Ray (on Leave) / S. Mass - 28" → S. Mass, 28  (substitute after "/")
 *   "Mulford-Roy 12"                   → Mulford-Roy, 12
 *
 * Rules: one teacher per non-empty line; parentheticals and the word
 * "student(s)" and grade ordinals ("4th") are noise; the last remaining
 * number is the student count; if a "/" splits two people, the person
 * after it (the current teacher/substitute) wins; the last word becomes
 * the last name. Emails are extracted in order and paired by position.
 */
