...
describe("pendingSendDate", () => {
  it("returns workshop minus 60 days for a never-sent school with a future due date", () => {
    // Oct 9 workshop → email due Aug 10.
    expect(pendingSendDate("2026-10-09", "never_sent", "2026-08-01")).toBe("2026-08-10");
    // Nov 6 workshop → email due Sep 7.
    expect(pendingSendDate("2026-11-06", "never_sent", "2026-08-20")).toBe("2026-09-07");
  });
  ...
  it("keeps showing an overdue date while the workshop is still 30+ days away", () => {
    ...
    expect(pendingSendDate("2026-10-09", "never_sent", "2026-08-20")).toBe("2026-08-10");
  });
  ...
  it("returns null once the workshop is less than 30 days away", () => {
    ...
    expect(pendingSendDate("2026-09-04", "never_sent", "2026-08-20")).toBeNull();
  });

  it("cuts off exactly at the 30-day boundary", () => {
    // Sep 19 workshop: cutoff day is Aug 20 — still shown on the boundary…
    expect(pendingSendDate("2026-09-19", "never_sent", "2026-08-20")).toBe("2026-07-21");
    // …and gone the day after.
    expect(pendingSendDate("2026-09-19", "never_sent", "2026-08-21")).toBeNull();
  });

  it("returns null for schools that were already emailed", () => {
    expect(pendingSendDate("2026-10-09", "sent_waiting", "2026-08-01")).toBeNull();
    expect(pendingSendDate("2026-10-09", "answered", "2026-08-01")).toBeNull();
  });

  it("returns null without a workshop date or with a malformed one", () => {
    expect(pendingSendDate(null, "never_sent", "2026-08-01")).toBeNull();
    expect(pendingSendDate("not-a-date", "never_sent", "2026-08-01")).toBeNull();
  });
});
