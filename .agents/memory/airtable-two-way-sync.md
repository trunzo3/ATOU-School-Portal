...
- **Workshop time normalizes on pull, compares by clock value.** Airtable's free time text ("8:15am - 9:45am") is stored in the portal as its normalized "HH:MM" start time when it parses (raw when not); the per-field baseline stays Airtable's RAW text, and the merge compares workshop-time values by normalized clock equivalence — otherwise the portal's "08:15" reads as a portal edit and clobbers Airtable's human-readable range on every pass. Unparseable-but-Airtable-entered current answers get a guarded backfill row when they normalize; portal-entered answers are never rewritten.
  **Why:** the time view (Start Time picker / schedule / printable form) only understands strict "HH:MM" (parseHM), while Airtable holds human-readable text that must never be clobbered by a push-back loop.
