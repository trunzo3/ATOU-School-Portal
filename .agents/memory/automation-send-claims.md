...
- Auto logistics: settings are a list of up to two rules (template + daysBefore). Semantics: the request template is first-contact only (zero prior sends of any kind); the follow-up template needs ≥1 prior send, no prior follow-up, AND logistics still incomplete. Validation forces the follow-up to have a strictly smaller daysBefore than the request (else it would never fire, or double-email on a shared day); follow-up-only is allowed on purpose (covers manually-emailed schools). Per school+rule, one transaction takes pg_advisory_xact_lock(823001, schoolId), re-checks the enabled switch + the exact rule still existing + school skip/lock + send history + (for follow-ups) completeness under the lock, and inserts the email_sends row with delivered=false. Delivery then runs outside the transaction; success flips delivered=true. A failure or crash leaves a visible "Not delivered" row and the school is never retried (exactly-daysBefore rule).
